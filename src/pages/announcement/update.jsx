...
import { reqUpdateAnnouncement, reqSchoolByRealname } from '../../api';
...
const AnnouncementUpdate = (props) => {
  ...
  const UpdateAnnouncement = async (values) => {
    ...
    // 2. 提交添加的请求
    const announcementId = props.history.location.state._id;  // 得到当前行公告的id
    const result = await reqUpdateAnnouncement({announcementObj, announcementId});
    // 3. 更新列表显示
    if (result.status === 0) {
  ...
  // 得到回显的 announcementObj
  const announcementObj = props.history.location.state;
  const { pub_theme, pub_content } = announcementObj || {};
